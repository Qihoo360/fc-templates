...
echo "========================================"
echo "配置信息："
echo "- 后端地址: ${YPROMPT_HOST}:${YPROMPT_PORT}"
echo "- 域名: ${DOMAIN}"
...
$(if [ "$SSL_AVAILABLE" = true ]; then
    ...
    # API请求 - 代理到Python后端
    location /api/ {
        proxy_pass http://${YPROMPT_HOST}:${YPROMPT_PORT}/api/;
...
start_backend() {
    ...
    # 将后端日志输出到统一的日志目录
    python3 run.py > /app/data/logs/backend/yprompt.log 2>&1 &
    BACKEND_PID=$!
    
    echo "✓ 后端服务已启动 (PID: $BACKEND_PID)"
    echo "✓ 日志文件: /app/data/logs/backend/yprompt.log"
...
monitor_backend() {
    ...
    while true; do
        ...
        if ! kill -0 $BACKEND_PID 2>/dev/null; then
            ...
            tail -n 20 /app/data/logs/backend/yprompt.log
...
echo "等待后端服务启动..."
for i in $(seq 1 30); do
    if curl -sf --max-time 5 http://${YPROMPT_HOST}:${YPROMPT_PORT}/api/auth/config >/dev/null 2>&1; then
    ...
    if [ $i -eq 30 ]; then
        ...
        echo "最后的日志输出:"
        tail -n 50 /app/data/logs/backend/yprompt.log
        exit 1
    fi
    echo "等待后端服务... ($i/30)"
    sleep 1
